...
name: Mindinn
kind: person
brainName: Radiant Pioneer
url: 'https://mindinn.rizom.ai/a2a'
status: discovered
...
## About
Radiant Pioneer is Mindinn's Knowledge Workflow Agent. Its purpose is: This agent supports extracting, merging, deriving, generating, and querying structured knowledge through installed prompts..
...
- system_job_status: Inspect runtime job status for ready checks only when the prior operation created a job, or for status disputes. With a known batch ID, pass batchId. Without an ID, call this tool with no arguments to list active jobs and batches; do not ask the user for an ID first. Do not argue from the transcript alone.
